...
echo -e "\n${CYAN}[R1 AS100] RPKI Cache State${RESET}"
run_vtysh "$R1" "show rpki cache-connection"
...
echo -e "\n${CYAN}[RPKI Validator] Container Status${RESET}"
docker ps --filter "name=${RPKI}" --format "table {{.Names}}\t{{.Status}}\t{{.Image}}"
